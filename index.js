...
function saveCookies(cookieString) {
  ...
  // Ensure the file is 0o600 even if it was previously created with broader permissions
    ...
    Object.entries(methods)
      ...
      .map(([method, op]) => {
        ...
        return {
          ...
          requestBody:
            op.requestBody
              ? summarizeSchema(op.requestBody.content?.["application/json"]?.schema, spec, undefined, schemaCache)
              : bodyParam?.schema
              ? summarizeSchema(bodyParam.schema, spec, undefined, schemaCache)
              : null,
...
function summarizeSchema(schema, spec, seen = new Set(), cache = new Map()) {
  ...
  if (schema.$ref) {
    ...
    const summarized = summarizeSchema(resolved, spec, newSeen, cache);
    cache.set(schema.$ref, summarized);
    return summarized;
  }
